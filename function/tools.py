...
tools = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "memory_type":{
                        'type':'string',
                        'description':'记忆的类型,其中PATH专门用来存储图像路径,DIAGNOSIS用来记录摘要之后的诊断记录',
                        'enum':['FACT','DIAGNOSIS','IMPORTANT','PATH']
                    },
                    'operation_type':{
                        'type':'string',
                        'description':'操作的类型',
    ...
    #调用多模态大模型
    {
        ...
        "function": {
            ...
            "3.用户能看到多模态的输出,不必复述"+\
            '4.对比时左图是当前会话的图,右图是你从数据库上传的图'+\
            '5.多模态模型无记忆,只要求它进行视觉相关的分析,其余分析你根据视觉分析的结果来接着分析',
            "parameters": {
                "type": "object",
                "properties": {
                    "question": {
